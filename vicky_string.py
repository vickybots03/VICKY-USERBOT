...
def main():
    print("T E A M    VICKY B O T   ! !")
    print("Hello!! Welcome to VickyBot Session Generator\n")
...
def generate_hellbot_session():
    print("!!! VickyBot SESSION !!!")
    print("One session for all VickyBot Project.")
    api_id = int(input("\nEnter APP ID here: "))
    api_hash = input("\nEnter API_HASH here: ")
    with PClient(name="helluser", api_id=api_id, api_hash=api_hash, in_memory=True) as hell:
        print("\nYour VICKYBOT SESSION is saved in your telegram saved messages.")
        _session = vickybot.export_session_string()
        vicky_session = vickybot_session(_session)
        vicky.send_message(
            "me",
            f"#VICKYBOT_SESSION \n\n`{vicky_session}`",
        )
...
def generate_pyro_session():
    ...
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with PClient(name="vickyuser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as hellbot:
        print("\nYour VickyBot Session Is sent in your Telegram Saved Messages.")
        vickybot.send_message(
            "me",
            f"#VICKYBOT #PYROGRAM\n\n`{vickybot.export_session_string()}`",
        )
...
def generate_insta_session():
    print("Instagram Session For VickyBot!")
    cl = IClient()
...
def vickybot_session(session):
    ...
    if len(session) in pyro_format.keys():
        ...
        return f"=={vickybot('vicky)}{new_session}{vickybot('bot')}=="
    else:
        return error_msg
